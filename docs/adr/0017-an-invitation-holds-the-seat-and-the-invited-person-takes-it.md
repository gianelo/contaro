...
Nothing is emailed, and that is settled rather than postponed. The invitation waits on the Space list until the person signs in and looks, which is enough for a product two people share and not enough for one where they do not already know it is coming.

#27 was opened to decide whether to close that gap, and it was closed `wontfix`. The two people who share a Space already talk to each other: "Te invité, entrá" is a message that was going to be sent anyway, on a channel they already use, and it arrives faster and gets read more reliably than mail would. The mail costs a transactional provider, a verified sending domain, SPF/DKIM/DMARC records, deliverability that decays the moment nobody watches it, a template that has to survive every mail client, and a seam so that tests, CI and preview deploys never reach a real person — which would extend ADR-0009 rather than follow it, because that decision keeps production data out of a preview and says nothing about what a preview sends outwards. That is recurring infrastructure bought to replace a message somebody sends by hand in a second.

Nothing in this decision depended on the mail, because the row is already the invitation. What it does contradict is the wording of story 8 in #1 — "so that my partner can share it the way a document is shared" — because sharing a document sends mail. The story's intent is met and its mechanism is not: the seat is offered to an address, and the person is told by whoever invited them. If that ever stops being true, sending mail is a new ticket.
